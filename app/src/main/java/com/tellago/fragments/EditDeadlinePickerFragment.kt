...
//        numPicker_day.maxValue = 29 // max value is 28 to protect DateTimeFormat conversion logic
//    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        numPick_year.value = calDiff.get(Calendar.YEAR) - 1970
        numPick_month.value = calDiff.get(Calendar.MONTH)
        numPick_day.value = calDiff.get(Calendar.DAY_OF_MONTH) - 1

        btn_confirm_deadline_edit.setOnClickListener {
            val deadline = Calendar.getInstance(timezone, locale)

            deadline.add(Calendar.YEAR, numPick_year.value)
            deadline.add(Calendar.MONTH, numPick_month.value)
            deadline.add(Calendar.DAY_OF_MONTH, numPick_day.value)
